...
type StockRow = { warehouse: string; year: number; lot: Lot; size: Size; qty_ml: number; approx_units: number };
...
export default function HomeClient() {
  ...
  async function loadStock(warehouseName?: string) {
    let query = getSupabase().from('v_stock_detailed').select('*');
  ...
  return (
    <form onSubmit={submit} className="mt-6 grid grid-cols-2 md:grid-cols-6 gap-3 items-end">
      <div className="flex flex-col">
        ...
        <select className="border rounded p-2" value={warehouseId}
      ...
      <div className="flex flex-col">
        ...
        <select className="border rounded p-2" value={year}
      ...
      <div className="flex flex-col">
        ...
        <select className="border rounded p-2" value={lot}
      ...
      <div className="flex flex-col">
        ...
        <select className="border rounded p-2" value={size}
      ...
      <div className="flex flex-col">
        ...
        <select className="border rounded p-2" value={type}
          onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setType(e.target.value as Movement)}>
          <option value="in">Ingresso</option><option value="out">Uscita</option><option value="adjustment">Rettifica</option>
        </select>
      </div>
      <div className="flex flex-col">
        <label className="text-sm">Quantità (unità)</label>
        <input className="border rounded p-2" type="number" min={1} value={units}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setUnits(Math.max(1, parseInt(e.target.value || '1', 10)))} />
      </div>
      <div className="md:col-span-4 flex flex-col">
        <label className="text-sm">Note</label>
        <input className="border rounded p-2" value={note}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNote(e.target.value)} placeholder="es. carico 50 casse" />
      </div>
